...
if (viewMode === "stacked") {
  if (stackedJobs && stackedJobs.length > 0) {
    console.log('Processing stacked jobs:', stackedJobs.length);
    
    stackedJobs.forEach(job => {
      if (job.jobName && job.notes && Array.isArray(job.notes)) {
        notesByJob[job.jobName] = job.notes.map(note => ({
          ...note,
          id: note.id || `temp-${Math.random()}`,
          userName: note.userName || 'Unknown User',
          documentCount: note.documentCount || 0,
    ...
    console.log("No stacked jobs data available from API");
  }
} else {
